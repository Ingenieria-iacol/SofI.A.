...
function restaurarEstado(jsonState) {
    ...
    if(typeof window.renderScene === 'function') window.renderScene();
    if(typeof window.updatePropsPanel === 'function') window.updatePropsPanel();
}
...
window.addEl = function(data) { 
    ...
    window.elementos.push({
        id: Date.now(), layerId: window.activeLayerId, visible: true, ...data
    }); 
    window.saveState(); 
    if(typeof window.renderScene === 'function') window.renderScene(); 
}
...
window.setTool = function(t) { 
    ...
    const activeBtn = document.getElementById('btn-'+t); if(activeBtn) activeBtn.classList.add('active'); 
}
...
window.cortarTuberia = function(idTuberia, xCorte, yCorte, zCorte) {
    ...
    window.addEl({ 
        tipo: 'tuberia', x: xCorte, y: yCorte, z: zCorte, dx: dx2, dy: dy2, dz: dz2, 
...
window.mostrarInputDinámico = function(xScreen, yScreen, distActual, vectorData) {
    ...
    input.value = (distActual * u.factor).toFixed(u.precision); 
...
window.handleCanvasClick = function(e) {
    ...
    if(window.estado.tool === 'select') { 
        ...
        const pickedID = selectElementAt(mx, my); // Uso de funcion local
        ...
        if(window.estado.selID) { 
            ...
            if(el) { 
                ...
                if(typeof window.syncZInput === 'function') window.syncZInput(); 
            } 
        }
        if(typeof window.updatePropsPanel === 'function') window.updatePropsPanel(); 
        if(typeof window.renderEffects === 'function') window.renderEffects();
...
// RESTO DE FUNCIONES (Guardar, Cargar, Reporte, PDF) - Se mantienen igual
...
window.guardarEnNavegador = function() { 
    try { 
        ...
        if(msg) { msg.style.display = 'block'; setTimeout(() => { msg.style.display = 'none'; document.getElementById('modal-guardar').style.display = 'none'; }, 1500); }
...
window.cargarProyecto = function(inputElement){ 
    ...
    r.onload = function(e) {
        try {
            ...
            window.saveState(); 
            if(typeof window.renderScene === 'function') window.renderScene(); 
            if(typeof window.renderLayersUI === 'function') window.renderLayersUI();
        } catch(err) { alert("Error al leer el archivo."); }
    }; 
    r.readAsText(inputElement.files[0]); 
}

window.limpiarTodo = function(){ if(confirm("¿Estás seguro de borrar todo?")){ window.elementos = []; window.saveState(); if(typeof window.renderScene === 'function') window.renderScene(); } }
...
window.mostrarReporte = function() {
    ...
    window.elementos.forEach(el => {
        ...
        } else if (el.tipo === 'cota' || el.tipo === 'texto') { return; } else {
            let cat = "Otros";
...
window.exportarCSV = function() {
    let csvContent = "data:text/csv;charset=utf-8,Categoria,Elemento,Cantidad/Longitud,Unidad\r\n";
    window.elementos.forEach(el => {
