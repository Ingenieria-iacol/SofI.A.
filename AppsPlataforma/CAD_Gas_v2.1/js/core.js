...
// --- TOPOLOGÍA Y CÁLCULO DE RED (NUEVO V2.5) ---
...
window.moverConConexiones = function(idElemento, dx, dy, dz) {
    const el = window.elementos.find(e => e.id === idElemento);
    if (!el) return;
...
window.cortarTuberia = function(idTuberia, xCorte, yCorte, zCorte) {
    const el = window.elementos.find(e => e.id === idTuberia);
...
window.handleCanvasClick = function(e) {
    ...
    if (window.estado.drawing && window.estado.inicio && !window.estado.snapped) {
        ...
        else if (Math.abs(dx) < th && Math.abs(dz) < th) { tx = window.estado.inicio.x;
        tz = window.estado.inicio.z; ty = gridY; } 
        else if (Math.abs(dx) < th && Math.abs(dy) < th) { tx = window.estado.inicio.x;
        ty = window.estado.inicio.y; tz = window.estado.inicio.z; }
        else { tx = gridX;
        ty = gridY; }
    }
...
window.limpiarTodo = function(){ 
    if(confirm("¿Estás seguro de borrar todo?")){ window.elementos = [];
    window.saveState(); window.cerrarPropiedades(); renderScene(); } 
}

// Funciones de reporte
window.mostrarReporte = function() {
    const tuberias = {};
    const accesorios = {}; const equipos = {};
    window.elementos.forEach(el => {
        if (el.visible === false) return;
        if (el.tipo === 'tuberia') {
...
window.generarPDF = function() { alert("Simulado");
